...
var _lambdaize = function (func) {
    ...
    var _lambda = function(event, context) {
        ...
        // TODO Remove these before commit
        
        var sqs_svc = new AWS_sdk.SQS();

        var sendToSqs = function(data, afterSent) {
        ...
        var _newCallback = function () {
            ...
            if(executionSucceeded) {
                sendToSqs({success: true, data: arguments}, finishExecution);
            } 
            else {
                sendToSqs({success: false, data: executionError}, finishExecution);
            }
        };
        ...
        try {
            ...
            executionError = JSON.parse(JSON.stringify(error, ['stack', 'message'], 2));
            _newCallback();
        }
    };
...
var _uploadFunctionAsync = function (lambdaFunc, config) {
    ...
    fs.writeFileSync("/Users/slvn/Desktop/projects/node/lambdaws/test/out.js", params.FunctionZip);
...
exports.create = function(func, config) {
    ...
    var proxy = function () {
        // TODO Check if last argument is a callback, throw if it's not
        // TODO remove the callback from arguments because we replace it
        // TODO Invoke remote function (we got it from the upload). If upload not finished wait for it first.
        // TODO Get the RequestId back from the invokation (look Lambda documentation)
        // TODO Register the RequestId with the original callback

        /*
            TODO When invoking lambda, we need to pass it:
            {
                args: arguments,
                sqsQueueUrl: null, // todo the queue url
            }
        */
