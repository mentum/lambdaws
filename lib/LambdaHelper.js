...
var MODULE_HASH_PREFIX = 'fromModule',
...
var _uploadZipAsync = function(zipContent, config, functionIdentifier, isModule) {
    var handler;
    var handlerName = config.name || 'default';
    var functionName    = handlerName + '-' + functionIdentifier;
...
module.exports = function (aws, queueInitializedPromise, executionStore){
    ...
    this.getCloudedFunctionFromFunction = function (func, deps, configs) {
        ...
        if (!proxyStore.hasOwnProperty(functionIdentifier)) {
            ...
            var zippedFunction  = zipper.zipFunction(functionAsString, handlerName, deps);
                uploadPromise   = _uploadZipAsync(zippedFunction, functionConfig, functionIdentifier, false);
    ...
    this.getCloudedFunctionFromModule = function(modulePath, handlerName, deps, configs) {
        ...
        if(!proxyStore.hasOwnProperty(functionIdentifier)) {
            ...
            var uploadPromise   = _uploadZipAsync(zippedModule, functionConfig, functionIdentifier, true);
